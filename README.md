...
1. Split text on `，,。.` into segments. **Skip** a segment if it contains **ASCII digits or Latin letters**, if it is not **pure CJK unified ideographs**, or if it has **fewer than 5 Hanzi** (see `MIN_EVAL_HANZI_CHARS` in `text_pipeline.py`).
...
4. Report **sentence exact match rate** and **macro CER** (total edit distance over total gold Hanzi length). Before comparing prediction vs gold, optional **synonym normalization** applies (default rules + [`data/eval_synonyms.json`](data/eval_synonyms.json)): e.g. 其它→其他, 他/她/它→他, 的/地/得→的. Override with `--eval-synonyms path/to.json`.
...
| `benchmark_all_corpus_<ts>_scheme_compare.txt` | 各方案**独有判对**及相对每个其他方案的**多判对**句列表（可读文本） |
